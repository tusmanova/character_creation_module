...
def defence(char_name: str, char_class: Union[str, None]) -> str:
    ...
    if char_class == 'mage':
        m_damage: int = 10 + randint(-2, 2)
        return (f'{char_name} блокировал {m_damage} урона')
    if char_class == 'healer':
        h_damage: int = 10 + randint(2, 5)
        return (f'{char_name} блокировал {h_damage} урона')
...
def special(char_name: str, char_class: Union[str, None]) -> str:
    ...
    if char_class == 'warrior':
        stamina: int = 80 + 25
        return (f'{char_name} {text} «Выносливость {stamina}»')
    if char_class == 'mage':
        attack: int = 5 + 40
        return (f'{char_name} {text} «Атака {attack}»')
    if char_class == 'healer':
        defense: int = 10 + 30
        return (f'{char_name} {text} «Защита {defense}»')
